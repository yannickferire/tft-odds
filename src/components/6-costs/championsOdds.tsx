...
const ChampionsOdds: React.FC<IChampionsOdds> = ({ champs, setChamps, selectedLevel, traits, getOddsForStage, selectedStage, levelTen, viktor }) => {
  const getPool = () => {
    const newPool: any = [];
        ...
        {selectedChampions.map((champion) => {
          return (
              <li key={champion.name} className="flex flex-wrap border-b border-dashed border-midday py-6 content-center last:border-0">
                <ChampionOdds champs={champs} setChamps={setChamps} champion={champion} selectedLevel={selectedLevel} pool={pool} traits={traits} getOddsForStage={getOddsForStage} selectedStage={selectedStage}
            levelTen={levelTen}
            viktor={viktor} />
              </li>
        )})}

...
const Header = () => {
  ...
  return (
    <header className="flex flex-col text-center md:flex-row mb-8">
        ...
        <div className="flex items-end">
          ...
          <p className="bg-earlynight px-2 py-1 ml-3 -mb-px rounded text-xs">
            <span className="flex">
              ...
              {stateSet === 'pbe' ? <small className="ml-1 opacity-50">– PBE</small> : <small className="ml-1 opacity-50">– Patch {patch}</small>}</span> <a className="ml-1 opacity-40 hover:opacity-80 transition-all" href="https://tftodds.com"><span className="top-px relative">→</span> Set 9</a>
            </span>
          </p>
        </div>
      ...
      <div className={`md:hidden fixed top-0 ${mobileMenuOpen ? "left-0" : "left-full"} p-6 w-screen h-screen bg-midnight transition-all duration-500 z-40`}>
        <ul className="relative top-20 flex flex-col text-left justify-end gap-2 md:gap-6">
          <li>
            <Link 
              href="/" 
              className={`py-2 inline-block text-crema transition-all duration-300 ease-in-out cursor-pointer font-bold border-b-2 border-midnight ${router.pathname === '/' ? 'text-morning underline underline-offset-4 cursor-default' : ''}`}
            >Champions</Link>
          </li>
          <li>
            <Link 
      ...
      <div className="flex-1 hidden md:block self-end">
        <ul className="flex justify-end gap-4 md:gap-6">
          ...
          <li>
            <Link 
              href="/augments-tier" 
              className={`relative z-20 text-crema transition-all duration-300 ease-in-out cursor-pointer font-bold border-b-2 border-midnight ${router.pathname === '/augments-tier' ? 'text-morning border-morning cursor-default' : 'hover:border-crema'}`}
            >Augments Tier</Link>
          </li>

...
import { calculateContestationCopies, getCostClasses } from '@/utils/rolling-odds-utils';

interface ContestationSliderProps {
  contestationLevel: number;
  setContestationLevel: (level: number) => void;
  championCost: number;
  playerLevel: number;
  championName: string;
}
...
const CONTESTATION_LEVEL_COLORS = [
 ...
 * - First level: no translation
 * - Last level: translate fully left
 * - Middle levels: translate half left (centered)
 */
...
export default function ContestationSlider({
  contestationLevel,
  setContestationLevel,
  championCost,
  playerLevel,
  championName
}: ContestationSliderProps) {
  // Calculate dynamic contestation levels with copies removed based on cost and level
  const contestationLevels = useMemo(
    () => CONTESTATION_LEVEL_COLORS.map(({ level, label, color }) => ({
      level,
      label,
      color,
      copies: calculateContestationCopies(level, championCost, playerLevel)
    })),
    [championCost, playerLevel]
  );

  const currentContestation = contestationLevels.find(({ level }) => level === contestationLevel);
  const sliderColor = currentContestation?.color ?? DEFAULT_COLOR;
  const { textClass } = getCostClasses(championCost);

  // Determine the label text based on champion name
  const isAnyCost = championName.toLowerCase().startsWith('any');
  const labelText = isAnyCost ? 'this champion' : championName;
      ...
      <div className="relative">
        ...
        <div className="absolute -top-2 left-2 z-10 flex items-center gap-1 backdrop-blur-lg px-1">
          ...
          <span className="text-xs text-neutral-400">
            <strong className={textClass}>{labelText}</strong> contestation
          </span>
        </div>
        ...
        <div className="bg-midnight border border-white/10 rounded-md px-4 h-12 flex items-center justify-center">
          <div className="relative w-full">
            <Slider
              value={[contestationLevel]}
              onValueChange={(values) => setContestationLevel(values[0])}
              min={MIN_LEVEL}
              max={MAX_LEVEL}
              step={1}
              className="w-full relative cursor-pointer py-4"
              style={{ '--slider-range-bg': sliderColor } as React.CSSProperties}
              aria-label={`${labelText} contestation level`}
            />

            {/* Dynamic labels */}
            <div className="absolute !-bottom-2.5 left-0 right-0 h-5 pointer-events-none">
              {contestationLevels.map(({ level, label, copies }) => {
                const isSelected = contestationLevel === level;
...
// Export contestation calculation function for use in other components
export { calculateContestationCopies };

...
const ChampionOdds: React.FC<IChampionOdds> = ({ champion, selectedLevel, pool, champs, setChamps, traits, getOddsForStage, selectedStage, levelTen, viktor }) => {
  ...
  return (
    <>
      <header className="animate-fadein flex flex-col mr-2 md:mr-4">
        <div 
          ...
          className="animate-fromleft w-20 h-20 lg:w-24 lg:h-24 relative bg-midnight rounded group cursor-pointer">
            <Image 
              className={`mb-1 h-full border-4 border-carry rounded group-hover:grayscale group-hover:opacity-25 transition duration-500`}
              src={champion.image} 
              alt={champion.name}
              width="96"
              height="96" />
            <span className={`group-hover:scale-100 group-hover:opacity-100 transition duration-500 scale-75 origin-center opacity-0 absolute -mt-1 ml-1 text-6xl top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 rotate-45 text-carry`}>+</span>
          <p className={`absolute text-crema text-sm px-2 pb-0.5 text-center rounded bottom-0 bg-carry group-hover:grayscale group-hover:opacity-25 transition duration-500`}> <GoldIcon color="crema" size={2.5} /> {champion.cost}</p>
        </div>
      ...
      <div className="animate-fadein animate-delay-2 order-3 lg:order-2 lg:mr-4 mt-6 lg:mt-0 flex flex-1 md:flex-1 flex-col sm:flex-row lg:flex-wrap gap-4 lg:gap-1 lg:min-w-[165px] lg:w-40 lg:flex-none">
      ...
      <div className="animate-fadein animate-delay-1 order-2 lg:order-3 flex-1">
        <div className="flex flex-auto lg:flex-1">
          <OddsByStar star={1} champion={champion} selectedLevel={selectedLevel} pool={pool} ownedCopies={ownedCopies} opponentsCopies={opponentsCopies} sameCostCopies={sameCostCopies} emoji={randomizedEmojis[0]} getOddsForStage={getOddsForStage} selectedStage={selectedStage}
            levelTen={levelTen}
            viktor={viktor} />
          <OddsByStar star={2} champion={champion} selectedLevel={selectedLevel} pool={pool} ownedCopies={ownedCopies}opponentsCopies={opponentsCopies} sameCostCopies={sameCostCopies} emoji={randomizedEmojis[1]} getOddsForStage={getOddsForStage} selectedStage={selectedStage}
            levelTen={levelTen}
            viktor={viktor} />
          {/* <OddsByStar star={3} champion={champion} selectedLevel={selectedLevel} pool={pool} ownedCopies={ownedCopies}opponentsCopies={opponentsCopies} sameCostCopies={sameCostCopies} emoji={randomizedEmojis[2]} getOddsForStage={getOddsForStage} selectedStage={selectedStage}
            levelTen={levelTen}
            viktor={viktor} /> */}
        </div>
      </div>

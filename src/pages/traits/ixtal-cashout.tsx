...
const Ixtal: NextPage = () => {
  ...
  return (
    <>
      <Head>
        <title>Ixtal cashout rewards TFT odds Set {currentSet}{setStage === 2 ? '.5': null}</title>
        <link rel="canonical" href="https://tftodds.com/traits/ixtal-cashout" />
        <meta name="description" content="Discover the Ixtal cashout rewards in Teamfight Tactics Set 16. View complete odds, costs, and rewards for all 20 cashout levels!" />
        <meta property="og:title" content={`Ixtal cashout rewards TFT odds Set ${currentSet}${setStage === 2 ? '.5': ''}`} />
		    <meta property="og:description" content="Discover the Ixtal cashout rewards in Teamfight Tactics Set 16. View complete odds, costs, and rewards for all 20 cashout levels!" />
        <meta property="og:image" content="https://tftodds.com/share.jpg" />
        <meta property="og:url" content="https://tftodds.com/traits/ixtal-cashout" />
        <meta property="og:type" content="website" />
        <meta name="twitter:card" content="summary_large_image" />
		    <meta name="twitter:site" content="@tftodds" />
        <meta name="twitter:title" content={`Ixtal cashout rewards TFT odds Set ${currentSet}${setStage === 2 ? '.5': ''}`} />
        <meta name="twitter:description" content="Discover the Ixtal cashout rewards in Teamfight Tactics Set 16. View complete odds, costs, and rewards for all 20 cashout levels!" />
      ...
      <h1 className="text-3xl mt-4 mb-4 font-bold px-4 text-center">
        ...
        <strong className="text-morning">Ixtal</strong> cashout tables
      </h1>
      <div className="py-4 px-6 bg-earlynight rounded-lg max-w-xl w-full mx-auto mb-8 mt-4">
        <p className="text-left mb-4 text-sm leading-snug">
          Completing a Quest before combat ends grants you a new one at the start of next round.<br/>
          <span className="text-crema/50">
            (3) Complete Quests to earn Sunshards (<Image src="/images/icons/ixtal-shard.svg" alt="Sunshard" width={14} height={14} className="opacity-75 inline-block -mt-0.5" />) and spend them for loot!<br/>
            (5) Heal 2 player health at the start of each player combat.<br/>
            (7) After winning combat, find 50 <Image src="/images/icons/ixtal-shard.svg" alt="Sunshard" width={14} height={14} className="opacity-75 inline-block -mt-0.5" /> and unearth bonus loot.
          </span>
        </p>
        <TraitChampionsList champions={champs} traitName="Ixtal" />
      </div>
      <article className="flex flex-col text-sm max-w-2xl mx-auto mb-6 px-4 gap-2">
        ...
        <Table className="w-full mb-12">
          <TableHeader>
            <TableRow className="!border-b !border-crema !border-opacity-20">
              ...
              <TableHead className="text-center w-28"><Image src="/images/icons/ixtal-shard.svg" alt="Sunshard" width={16} height={16} className="inline-block -mt-0.5 mr-1" /></TableHead>
              <TableHead className="text-left">Possible Rewards</TableHead>
              <TableHead className="text-center w-24">Odds</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {Object.entries(ixtalCashouts).flatMap(([key, cashout]) => {
              const rowCount = cashout.options.length;
              const rowSpan = rowCount === 1 ? undefined : rowCount;

              return cashout.options.map((option, index) => {
                const uniqueKey = `${key}-option-${option.optionNumber}`;
                const costValue = cashout.cost === -1 ? 1 : cashout.cost;

                return (
                  <TableRow key={uniqueKey} className={`border ${index === 0 ? "border-t-midday border-t-2": ""}`}>
                    {index === 0 && (
                      <>
                        <TableCell
                          rowSpan={rowSpan}
                          className="py-1.5 text-xl text-center font-bold border border-crema border-opacity-20 bg-midnight"
                        >
                          {cashout.cashoutNumber}
                        </TableCell>
                        <TableCell
                          rowSpan={rowSpan}
                          className="py-1.5 text-center border border-crema border-opacity-20 bg-midnight leading-snug"
                        >
                          <span className="font-semibold text-base flex items-center justify-center">
                            {costValue}
                            <Image src="/images/icons/ixtal-shard.svg" alt="Sunshard" width={14} height={14} className="inline-block" />
                          </span>
                          <span className="text-xs opacity-70">({cashout.estimatedValue}g value)</span>
                        </TableCell>
                      </>
                    )}
                    <TableCell className="py-1.5 border border-crema border-opacity-20">
                      <FormatConsumablesWithTooltip value={option.rewards.join(' + ')} />
                    </TableCell>
                    <TableCell className="py-1.5 font-semibold text-center text-base border border-crema border-opacity-20">
                      {option.probabilityPercent}
                    </TableCell>
                  </TableRow>
                );
              });
        ...
        <h2 id="sevent-ixtal" className="text-3xl mt-4 mb-2 font-bold px-4 text-center"><strong className="text-morning">7 Ixtal bonus loot</strong></h2>
        <p className="text-center mb-4">(7) After winning combat, find 50<Image src="/images/icons/ixtal-shard.svg" alt="Sunshard" width={14} height={14} className="opacity-75 inline-block -mt-0.5" /> and unearth <strong>bonus loot</strong>.</p>
        <Table className="w-full mb-24">
          ...
          <TableBody>
            {bonusIxtalCashouts.map((bonus, index) => (
              <TableRow key={index}>
                <TableCell className="py-1.5 border border-crema border-opacity-20">
                  <FormatConsumablesWithTooltip value={bonus.rewards.join(' + ')} />
                </TableCell>
                <TableCell className="py-1.5 font-semibold text-center text-base border border-crema border-opacity-20">
                  {bonus.probabilityPercent}
                </TableCell>
              </TableRow>
            ))}
          </TableBody>
